...
// Extended interface with enhanced methods
interface EnhancedPluginSandboxInstance extends IPluginSandboxInstance {
  getMetrics: () => any
  resetMetrics: () => void
  isExpired: () => boolean
...
export class EnhancedPluginSandbox implements IPluginSandbox {
  private sandboxes = new Map<string, EnhancedPluginSandboxInstance>()
  ...
  async create(plugin: any): Promise<EnhancedPluginSandboxInstance> {
    ...
    const instance: EnhancedPluginSandboxInstance = {
      id: sandboxId,
  ...
  async execute(
    instance: EnhancedPluginSandboxInstance,
    code: string,
  ...
  async destroy(instance: EnhancedPluginSandboxInstance): Promise<void> {
    await this.destroySandbox(instance)
  }
    ...
    return {
      ...
      db: hasPermission('database', 'read')
        ...
        : {
            ...
            transaction: async () => ({
              error: 'Access denied: database:read permission required',
            }),
          },
      ...
      events: {
        ...
        on: async (event: string, _handler: Function) => {
          console.log('[Sandbox Event Handler] Registered for: ' + event)
        },
        off: async (event: string, _handler: Function) => {
          console.log('[Sandbox Event Handler] Removed for: ' + event)
        },
        once: async (event: string, _handler: Function) => {
          console.log('[Sandbox Event Handler] Once registered for: ' + event)
        },
      },
      ...
      http: hasPermission('network', 'read')
        ...
        : {
            ...
            post: async (_url: string, _data?: any, _options?: any) => {
              throw new Error('Access denied: network:write permission required')
            },

            put: async (_url: string, _data?: any, _options?: any) => {
              throw new Error('Access denied: network:write permission required')
            },

            delete: async (_url: string, _options?: any) => {
              throw new Error('Access denied: network:write permission required')
            },
          },
      ...
      crypto: {
        ...
        encrypt: async (data: string, _key: string) => {
          console.log('[Sandbox Crypto] Encrypting data')
          return 'encrypted_' + data
        },

        decrypt: async (data: string, _key: string) => {
      ...
      time: {
        ...
        add: (date: Date, amount: number, _unit: string) =>
          new Date(date.getTime() + amount * 1000),
   ...
   */
  private createSandboxLogger(_pluginId: string, sandboxId: string) {
    return {
   ...
   */
  private createSandboxStorage(_pluginId: string, sandboxId: string) {
  ...
  private wrapPluginCode(_code: string, context: PluginContext, config: any): string {
    return (
      '(function() {' +
      "'use strict';" +
   ...
   */
  private async destroySandbox(instance: EnhancedPluginSandboxInstance): Promise<void> {
    try {
      const resourceMonitor = this.resourceMonitors.get(instance.id)
      if (resourceMonitor) {
        resourceMonitor.stop()
