...
const PLUGIN_TEMPLATES = {
  basic: {
    ...
    files: {
      'plugin.json': (pluginName) => JSON.stringify({
        name: pluginName,
        slug: pluginName.toLowerCase().replace(/[^a-z0-9-]/g, '-'),
        version: '1.0.0',
        description: 'Plugin ' + pluginName + ' generated by SaaS Plugin CLI',
      ...
      'README.md': (pluginName) => '# ' + pluginName + ' Plugin\n\n' +
        ...
        '2. Run `saas-plugin install ' + pluginName.toLowerCase() + '`\n' +
        '3. Configure and activate in the admin panel\n\n' +
        '## Development\n\n' +
        '```bash\n' +
        '# Watch for changes\n' +
        'saas-plugin dev ' + pluginName.toLowerCase() + '\n\n' +
        '# Build plugin\n' +
        'saas-plugin build ' + pluginName.toLowerCase() + '\n\n' +
        '# Test plugin\n' +
        'saas-plugin test ' + pluginName.toLowerCase() + '\n' +
        '```\n\n' +
        '## Configuration\n\n' +
      ...
      'package.json': (pluginName) => JSON.stringify({
        name: pluginName.toLowerCase().replace(/[^a-z0-9-]/g, '-'),
        version: '1.0.0',
        description: 'Plugin ' + pluginName + ' for SaaS Boilerplate',
        main: 'src/index.js',
        scripts: {
          dev: 'saas-plugin dev',
        ...
        keywords: ['saas', 'plugin', pluginName.toLowerCase()],
        author: 'Plugin Developer',
        license: 'MIT'
      }, null, 2)
    }
...
async function createPlugin(pluginName, options) {
  try {
    ...
    // Get template
    const selectedTemplate = PLUGIN_TEMPLATES[template]
    if (!selectedTemplate) {
...
async function installPlugin(pluginPath) {
  try {
    ...
    try {
      ...
      console.error('❌ Invalid plugin manifest at: ' + manifestPath)
      process.exit(1)
    }
...
async function validatePlugin(pluginPath) {
  try {
    ...
    console.error('❌ Failed to validate plugin:', error.message)
    process.exit(1)
  }
}
 ...
 * Get plugin info
 */
async function infoPlugin(pluginPath) {
  try {
    const manifestPath = path.join(pluginPath, 'plugin.json')
