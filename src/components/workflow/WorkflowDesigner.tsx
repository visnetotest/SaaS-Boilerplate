...
export default function WorkflowDesigner({ workflowId }: { workflowId?: string }) {
  ...
  const [canvasState, setCanvasState] = useState<{
    ...
    selectedStepId: string | null
    isDragging: boolean
    dragType: 'step' | 'connection' | 'line' | null
    scale: number
  ...
  const handleCanvasMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    setCanvasState((prev) => {
      ...
      if (prev.isDragging && prev.dragType === 'step' && prev.selectedStepId) {
        const stepIndex = prev.steps.findIndex((s) => s.id === prev.selectedStepId)
        ...
        updatedSteps[stepIndex] = {
          ...updatedSteps[stepIndex],
          position: {
            x: Math.round((x - prev.offset.x) / prev.scale),
            y: Math.round((y - prev.offset.y) / prev.scale),
          },
        }
        ...
        return {
          ...prev,
          steps: updatedSteps,
          connections: inferConnections(updatedSteps),
        }
      }
   ...
   */
  const getDefaultStepConfig = (stepType: string): Record<string, unknown> => {
    switch (stepType) {
      case 'data-entry':
        return {
          fields: [
            { name: 'input1', type: 'text', label: 'Input Field', required: true },
            { name: 'input2', type: 'email', label: 'Email Field', required: false },
          ],
        }
      case 'approval':
        return {
  ...
  const renderStep = (step: WorkflowStep) => {
    ...
    return (
      <div
        ...
        style={{
          left: `${(step.position?.x || 0) * canvasState.scale + canvasState.offset.x}px`,
          top: `${(step.position?.y || 0) * canvasState.scale + canvasState.offset.y}px`,
      ...
      >
        <div
          className={`bg-white rounded-lg border-2 border-gray-300 p-4 h-full flex items-center ${isSelected ? 'border-blue-500' : ''}`}
        >
          <div
            className={`w-8 h-8 rounded-full flex items-center justify-center text-2xl ${stepType.color}`}
          >
            {stepType.icon}
          </div>
          <div className='ml-3 flex-1'>
            <div className='font-semibold text-sm'>{stepType.name}</div>
        ...
        {/* Connection points */}
        <div
          className='absolute w-3 h-3 bg-blue-500 rounded-full cursor-crosshair'
          style={{
            right: `${-6 * canvasState.scale}px`,
  ...
  const renderConnection = (connection: Connection) => {
    const fromPoint = {
      x: connection.fromPoint.x * canvasState.scale + canvasState.offset.x,
      y: connection.fromPoint.y * canvasState.scale + canvasState.offset.y,
    }

    const toPoint = {
      x: connection.toPoint.x * canvasState.scale + canvasState.offset.x,
      y: connection.toPoint.y * canvasState.scale + canvasState.offset.y,
    }
        ...
        <div
          ...
          style={{
            backgroundImage: `
              ...
              linear-gradient(0deg, #f9fafb 50%, #e5e7eb 50%)
            `,
          }}
        >
          {/* SVG for connections */}
          ...
          >
            ...
            <g>{canvasState.connections.map(renderConnection)}</g>
          </svg>

          {/* Render steps */}
          {canvasState.steps.map(renderStep)}
